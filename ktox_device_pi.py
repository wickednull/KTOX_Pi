...
def _lock_load_from_config(data: dict):
    ...
    # Disable lock if no secret is stored
...
def _select_ss_gif() -> None:
    ...
    while True:
        ...
        if btn in ("KEY1_PIN","KEY3_PIN"): break
            ...
            _lock_ss_cache["path"] = None   # bust cache
            _lock_save_config()
...
def OpenLockMenu() -> None:
    """Lock settings menu — accessible from the main menu."""
    ...
    while True:
        ...
        if s == "Lock now":
            ...
            cur = int(lock_config.get("auto_lock_seconds") or 0)
...
def _stealth_sensor(ts):
    ...
    try:
        ...
        draw.text((2, y + 16), f"AQI {aqi} \xb7 {lux}lx",
...
def _stealth_sysmon(ts, _start=[None]):
    if _start[0] is None:
        _start[0] = ts
    uptime_s = int(ts - _start[0]) + 172800 + 50400
    sf = _stealth_fonts()
    ...
    def _bar(y, pct, col, warn=80):
        W = 50
        c = (220, 80, 40) if pct >= warn else col
...
# ── Theme registry ────────────────────────────────────────────────────────────
_STEALTH_THEMES    = [_stealth_clock_fallback, _stealth_sensor, _stealth_sysmon]
    ...
    def navigate(self, key):
        ...
        if key.startswith("pay_"):
            ...
            while True:
                ...
                elif btn == "KEY_RIGHT_PIN":                   pass
                elif btn == "KEY_UP_PIN":                      pass
                elif btn == "KEY_DOWN_PIN":                    pass
            return
        ...
        while True:
            ...
            if btn is None:                                continue
            elif btn == "KEY_DOWN_PIN":                    sel = (sel+1)%len(labels); continue
            elif btn == "KEY_UP_PIN":                      sel = (sel-1)%len(labels); continue
            elif btn in ("KEY_PRESS_PIN","KEY_RIGHT_PIN"):
                self.select = sel
                action = items[sel][1]
