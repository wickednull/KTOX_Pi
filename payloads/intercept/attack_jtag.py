...
def draw_ui(screen_state="main", message_lines=None):
    ...
    d.text((5, 5), "JTAG Attack", font=FONT_TITLE, fill=(30, 132, 73))
    ...
    if message_lines:
        ...
        d.text((5, 30), f"Config: {os.path.basename(OPENOCD_CONFIG)}", font=FONT, fill=(242, 243, 244))
        d.text((5, 100), "OK=Start", font=FONT, fill=(171, 178, 185))
        d.text((5, 110), "KEY1=Config", font=FONT, fill=(171, 178, 185))
    elif screen_state == "attacking":
        d.text((5, 50), "Running attack...", font=FONT_TITLE, fill=(212, 172, 13))
        d.text((5, 70), f"Config: {os.path.basename(OPENOCD_CONFIG)}", font=FONT, fill=(242, 243, 244))
