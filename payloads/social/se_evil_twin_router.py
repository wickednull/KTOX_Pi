#!/usr/bin/env python3
"""
KTOx *payload* – **Evil Twin (Router Phishing)**
===================================================
This payload sets up an Evil Twin attack to phish for router credentials.
It creates a fake Wi-Fi access point, redirects all DNS requests to a local
web server hosting a fake router login page, and captures any entered
credentials.

Features:
- Creates a fake Wi-Fi AP with a configurable SSID and channel.
- Uses `hostapd` and `dnsmasq` for AP and DHCP/DNS services.
- Hosts a customizable phishing portal (default router login page).
...
TEMP_CONF_DIR = "/tmp/ktox_eviltwin_router/"
