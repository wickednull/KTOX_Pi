#!/usr/bin/env python3
"""
KTOx *payload* example – **Show Buttons**
=============================================
This script lives in the ``payloads/`` folder of the KTOx project.
...
3. **Exit cleanly** when:
   ...
   * KTOx UI sends a *SIGTERM* signal when the payload is stopped from the
     menu.
...
# ----------------------------- Third‑party libs ---------------------------
# These come pre‑installed on KTOx; on a vanilla Pi OS you’d need:
...
try:
    ...
    while running:
        ...
        if pressed:
            if pressed == "KEY3":  # ← user wants to go back to KTOx
