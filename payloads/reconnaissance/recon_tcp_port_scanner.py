...
LOOT_DIR = os.path.join(KTOX_ROOT, 'loot', 'TCP_Port_Scanner')
...
# Load PINS from KTOx gui_conf.json
...
def draw_ui(screen_state="main"):
    img = Image.new("RGB", (128, 128), (10, 0, 0))
    d = ImageDraw.Draw(img)
    d.text((5, 5), "TCP Port Scanner", font=FONT_TITLE, fill=(30, 132, 73))
    ...
    if screen_state == "main":
        ...
        d.text((5, 25), f"Open Ports: {len(open_ports)}", font=FONT, fill=(212, 172, 13))
        y_pos = 40
...
def run_scan():
    ...
    # Save results to loot under KTOx
    try:
