...
def draw_ui(screen_state="main", message_lines=None):
    ...
    d.text((5, 5), "Follina Attack", font=FONT_TITLE, fill=(30, 132, 73))
    ...
    if message_lines:
        ...
        d.text((5, 30), f"Command: {COMMAND}", font=FONT, fill=(242, 243, 244))
        d.text((5, 100), "OK=Generate", font=FONT, fill=(171, 178, 185))
        d.text((5, 110), "KEY1=Command", font=FONT, fill=(171, 178, 185))
    elif screen_state == "generating":
        d.text((5, 50), "Generating document...", font=FONT_TITLE, fill=(212, 172, 13))
        d.text((5, 70), f"Command: {COMMAND}", font=FONT, fill=(242, 243, 244))
...
def run_attack():
    ...
    follina_path = os.path.join(KTOX_ROOT, "follina-exploit", "follina.py")
