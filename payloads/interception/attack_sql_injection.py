#!/usr/bin/env python3
"""
KTOx *payload* – **SQL Injection Attack**
==============================================
This payload automates the process of finding and exploiting SQL injection
vulnerabilities using sqlmap.

Features:
- Interactive UI for entering the target URL.
- Uses sqlmap to perform the attack.
...
def cleanup(*_):
    ...
    subprocess.run("killall sqlmap", shell=True)
...
def draw_ui(screen_state="main", message_lines=None):
    ...
    d.text((5, 5), "SQL Injection Attack", font=FONT_TITLE, fill=(30, 132, 73))
...
def run_attack():
    ...
    try:
        ...
        if process.returncode == 0:
            draw_ui(message_lines=["Attack successful!", "Check sqlmap output."])
        else:
...
if __name__ == "__main__":
    try:
        ...
        LCD.LCD_Clear()
        GPIO.cleanup()
        print("SQL Injection Attack payload finished.")
