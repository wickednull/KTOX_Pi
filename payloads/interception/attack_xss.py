#!/usr/bin/env python3
"""
KTOx *payload* – **Cross-Site Scripting (XSS) Attack**
=========================================================
This payload automates the process of finding and exploiting XSS
vulnerabilities using XSStrike.

Features:
- Interactive UI for entering the target URL.
- Uses XSStrike to perform the attack.
...
def cleanup(*_):
    ...
    subprocess.run("killall xsstrike", shell=True)
...
def draw_ui(screen_state="main", message_lines=None):
    ...
    d.text((5, 5), "XSS Attack", font=FONT_TITLE, fill=(30, 132, 73))
    ...
    if message_lines:
        ...
        d.text((5, 30), f"Target: {TARGET_URL}", font=FONT, fill=(242, 243, 244))
        d.text((5, 100), "OK=Start", font=FONT, fill=(171, 178, 185))
        d.text((5, 110), "KEY1=URL", font=FONT, fill=(171, 178, 185))
    elif screen_state == "attacking":
        d.text((5, 50), "Running attack...", font=FONT_TITLE, fill=(212, 172, 13))
        d.text((5, 70), f"Target: {TARGET_URL}", font=FONT, fill=(242, 243, 244))
...
def run_attack():
    ...
    try:
        ...
        if process.returncode == 0:
            draw_ui(message_lines=["Attack successful!", "Check XSStrike output."])
        else:
...
if __name__ == "__main__":
    try:
        ...
        LCD.LCD_Clear()
        GPIO.cleanup()
        print("XSS Attack payload finished.")
