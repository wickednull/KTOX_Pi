#!/usr/bin/env python3
"""
KTOx *payload* – **Find SSH Servers**
========================================
This payload scans the local network for active SSH servers (port 22).
It performs a basic port scan on all hosts within the local subnet to
identify machines running an SSH service.

Features:
- Interactive UI for selecting the network interface to scan.
- Allows editing of the target SSH port (default 22).
- Scans all hosts in the local subnet for the specified port.
- Displays found SSH server IP addresses on the LCD.
- Graceful exit via KEY3 or Ctrl-C.

Controls:
- MAIN SCREEN:
    - OK: Start scanning for SSH servers.
    - KEY1: Select network interface.
    - KEY2: Edit target SSH port.
    - KEY3: Exit Payload.
...
LOOT_DIR = os.path.join(KTOX_ROOT, 'loot', 'SSH_Servers')
...
def draw_ui(screen_state="main"):
    img = Image.new("RGB", (128, 128), (10, 0, 0))
    d = ImageDraw.Draw(img)
    d.text((5, 5), "Find SSH Servers", font=FONT_TITLE, fill=(30, 132, 73))
    d.line([(0, 22), (128, 22)], fill=(30, 132, 73), width=1)

    if screen_state == "main":
        with ui_lock:
            d.text((5, 25), f"Interface: {ETH_INTERFACE}", font=FONT, fill=(242, 243, 244))
            d.text((5, 40), f"Port: {SSH_PORT}", font=FONT, fill=(242, 243, 244))
            if "Scanning" in status_msg or "Press" in status_msg:
                d.text((5, 55), status_msg, font=FONT, fill=(212, 172, 13))
            else:
                d.text((5, 55), f"Servers Found: {len(ssh_servers)}", font=FONT, fill=(212, 172, 13))
        ...
        d.text((5, 115), "OK=Scan | KEY1=Edit Iface | KEY2=Edit Port | KEY3=Exit", font=FONT, fill=(171, 178, 185))
        ...
        d.text((5, 30), "Enter Port:", font=FONT, fill=(242, 243, 244))
