#!/usr/bin/env python3
"""
KTOx Payload – Nmap Network Scanner
====================================
Interactive network reconnaissance and port scanning using nmap.

Features:
  - Network discovery scanning
  - Port scanning (fast, stealth, comprehensive)
...
def _display_output(lines):
    ...
    draw.text((4, 1), "NMAP", font=ImageFont.load_default(), fill=(192, 57, 43))
...
def main():
    ...
    try:
        ...
        # Quick test to verify nmap is available
        proc.terminate()
        proc.wait(timeout=1)
    except Exception:
        img = Image.new("RGB", (WIDTH, HEIGHT), (10, 0, 0))
        draw = ImageDraw.Draw(img)
        draw.text((4, 20), "Nmap not found", fill=(242, 243, 244))
        LCD.LCD_ShowImage(img, 0, 0)
        time.sleep(3)
        GPIO.cleanup()
        return 1
    ...
    try:
        while running:
            ...
            if scanning:
                ...
                draw.rectangle((0, 0, 127, 12), fill=(139, 0, 0))
                draw.text((4, 1), "NMAP", font=ImageFont.load_default(), fill=(192, 57, 43))
                draw.text((2, 20), "Target:", font=ImageFont.load_default(), fill=(242, 243, 244))
                draw.text((2, 40), input_buffer[:20], font=ImageFont.load_default(), fill=(170, 170, 255))
                draw.text((2, 100), "OK=Scan  KEY3=Exit", font=ImageFont.load_default(), fill=(113, 125, 126))
                LCD.LCD_ShowImage(img, 0, 0)
            ...
            if btn == "KEY3":
                    ...
                    if kb.row >= 0:
                        ...
                        if key == "ENT":
                            if input_buffer.strip():
                                scanning = True
                                output_lines = []
                                reader = threading.Thread(target=run_nmap, daemon=True)
                                reader.start()
                        elif key == "BS":
                ...
                # Scanning mode: allow early exit
                if btn == "KEY3":
                    if proc:
                        try:
                            proc.terminate()
