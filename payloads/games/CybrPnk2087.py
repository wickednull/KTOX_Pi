...
def display_text(lines, title=None):
    ...
    y = 2
    if title:
        DRAW.text((2, y), title.upper(), font=FONT_BOLD, fill=(0, 255, 255))
        y += 12
        DRAW.line((0, y-2, W, y-2), fill=(255, 255, 255))
    for line in lines[:8]:
...
# ----------------------------------------------------------------------
class Combatant:
    def __init__(self, name, hp, attack, speed=10, defense=0,
...
HACK_MENU = [
    ...
    ("System Shock",   50, "Stun",     1, True),
]

def combat_encounter(player_crew, enemies_data, game, flee_allowed=True, boss_music=False):
    def _make_enemy(e):
    ...
    while any(e.is_alive() for e in enemies) and any(p.is_alive() for p in player_crew):
        ...
        if fighter in player_crew:
            ...
            if action == 0:
                ...
                if not alive_en:
                    continue
                t_idx = menu_choice([f"{e.name} {e.hp}/{e.max_hp}HP" for e in alive_en], "Attack who?")
                if t_idx is None:
                    return "hub"
                if t_idx == -1:
                    continue
            ...
            # Quickhack
                ...
                if not game.has_item("cyberdeck"):
                    ...
                    continue
                hack_labels = [f"{h[0]} ({h[1]}E)" for h in HACK_MENU] + ["Cancel"]
...
class Game:
    ...
    def open_inventory(self):
        while True:
            ...
            if choice in (-1, 4) or choice is None:
                return
            if choice == 0:
...
# =============================================================================
# SCENE FUNCTIONS (fully corrected indentation + KEY3 exit)
# =============================================================================
...
def scene_tanaka_gives_key(g):
    ...
    if idx == 0:
        ...
        g.show_text(["Tanaka: 'Then we have nothing to talk about.' She walks away."])
        return "act3_biokey"
    else:
        g.show_text(["Tanaka: 'Then I can't help you.' She leaves."])
        return "act3_biokey"
...
def scene_afterlife_hub(g):
    ...
    next_act = "Act 2" if g.story_act <= 2 else "Act 3" if g.story_act == 3 else "Act 4" if g.story_act == 4 else "Act 5" if g.story_act == 5 else "Act 6" if g.story_act == 6 else "Act 7" if g.story_act == 7 else "Act 8" if g.story_act == 8 else "Finale"
    g.show_text([
        "THE AFTERLIFE",
    ...
    if idx == 0:
        # story continue – we'll route based on story_act
        if g.story_act == 0:
            return "prologue"
        elif g.story_act == 1:
            return "act2_vector_lead" if g.check_flag("vector_ally") else "act2_no_vector"
        elif g.story_act == 2:
            return "act2_vector_lead" if g.check_flag("vector_ally") else "act2_no_vector"
        elif g.story_act == 3:
            return "act3_biokey"
...
def scene_street(g):
    ...
    if idx == 0:
        ...
        return "pacifica_side"
    elif idx == 4:
        return "watson_district"
    return "afterlife_hub"
...
def scene_kabuki(g):
    ...
    if idx == 0:
        return "vendor_netrunner"
    elif idx == 1:
        return "kabuki_cyberware"
    elif idx == 2:
...
def scene_fixer_gigs(g):
    ...
    if idx == -1 or idx == len(available):
        return "afterlife_hub"
    gig_key = available[idx]
    if gig_key == "ghost_data":
        return "gig_ghost_data"
    elif gig_key == "blood_money":
        return "gig_blood_money"
    elif gig_key == "broken_doc":
        return "gig_broken_doc"
    elif gig_key == "steel_nerves":
        return "gig_steel_nerves"
    elif gig_key == "dead_drop":
        return "gig_dead_drop"
    else:
        return "afterlife_hub"
...
def scene_gig_broken_doc_warehouse(g):
    ...
    has_intel = (g.check_flag("vik_trail_net") or g.check_flag("vik_trail_chrome_kaz"))
    idx = g.choose([
    ...
    if idx == 0:
        ...
        has_trade = (g.eddies >= 1000 or any(g.has_item(x) for x in ["smart_rifle","optical_camo","cyberdeck"]))
        if has_trade:
            idx2 = g.choose([
...
def scene_pacifica_ruins(g):
    ...
    if roll < 0.3:
        ...
        g.show_text([
            "An old netrunner den. Dead terminals. A single note:",
            "'We were here. We mattered. – L'"
        ])
        g.lucy_trust += 1
    return "afterlife_hub"
...
def main():
    ...
    # No GPIO.cleanup() – KTOx restores hardware
