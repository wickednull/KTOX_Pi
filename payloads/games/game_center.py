...
EMULATORS = {
    "nes": {"name": "NES (jsnes)", "apt": [], "web": "https://binji.github.io/binjnes/", "ext": [".nes"]},
    "gb": {"name": "Game Boy (Binjgb)", "apt": [], "web": "https://binji.github.io/binjgb/", "ext": [".gb", ".gbc"]},
    "doom": {"name": "DOOM (Chocolate Doom local)", "apt": ["chocolate-doom"], "web": "", "ext": [".wad"]},
}
...
</head><body><h2>KTOx Game Center</h2><p>Host: <code>{{ host }}</code></p>
<h3>Emulators</h3><ul>{% for key, e in emulators.items() %}<li><b>{{e.name}}</b>
<form style='display:inline' method='post' action='/install/{{key}}'><button>Install</button></form>
{% if e.web %}<a href='{{e.web}}' target='_blank'><button type='button'>Open Web Emulator</button></a>{% endif %}</li>{% endfor %}</ul>
<h3>ROM Manager</h3><form method='post' enctype='multipart/form-data' action='/rom/upload'>
<input type='file' name='rom_file'><button>Upload ROM</button></form><p><a href='/rom/list'>List ROMs (JSON)</a></p></body></html>"""
...
@APP.get("/")
def home():
    return render_template_string(TEMPLATE, host=request.host_url.rstrip("/"), emulators=EMULATORS)
...
def install_emu(emu):
    ...
    for cmd in (["apt-get", "update"], ["apt-get", "install", "-y", *meta["apt"]]) if meta["apt"] else []:
        rc, out = _run(cmd)
        logs.append({"cmd": " ".join(cmd), "rc": rc, "output": out[-1200:]})
...
def list_roms():
    ...
    for f in sorted(ROMS_DIR.rglob("*")):
        if f.is_file():
            rows.append({"name": f.name, "path": str(f.relative_to(ROMS_DIR)), "size": f.stat().st_size})
...
def download_rom(p):
    ...
    try:
        pth.relative_to(ROMS_DIR.resolve())
    except ValueError:
...
def _run_serve_mode():
    _ensure_dirs()
    print(f"[game_center] Starting web UI on 0.0.0.0:{PORT}", flush=True)
...
def _run_lcd_mode():
    ...
    while True:
        ...
        d.rectangle((0, 0, 128, 18), fill=(0, 110, 32) if running else (90, 25, 25))
        d.text((3, 2), 'Game Center', font=bold, fill='white')
        d.text((3, 21), 'RUNNING' if running else 'STOPPED', font=bold, fill=(80, 255, 140) if running else (255, 180, 80))
        d.text((3, 38), 'URL:', font=font, fill='yellow')
        for i, chunk in enumerate([url[j:j+21] for j in range(0, len(url), 21)][:3]):
            d.text((3, 49 + i * 10), chunk, font=font, fill='cyan')
        d.text((3, 82), f'PID: {pid if running else "-"}', font=font, fill='white')
        if msg:
            d.text((3, 95), msg[:22], font=font, fill=(255, 220, 120))
        d.line((0, 113, 128, 113), fill=(90, 90, 90))
        d.text((2, 116), 'OK=start DOWN=stop K3=back', font=font, fill='yellow')
        ...
        if btn in ("OK", "UP", "RIGHT"):
            msg = _start_server().get("message", "")
        elif btn in ("DOWN", "KEY1"):
            msg = _stop_server().get("message", "")
        time.sleep(0.12)
...
def main():
    ...
    try:
        ...
        # headless/dev fallback
        print("[game_center] LCD unavailable. Starting server directly.")
        _run_serve_mode()
