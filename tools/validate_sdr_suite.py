...
def validate_static_assets() -> None:
    ...
    for token in ["Dashboard", "Receiver", "Listen", "NFM", "WFM", "AM", "USB", "LSB", "receiverSpectrum", "receiverWaterfall", "rxStep", "rxBandwidth", "rxSquelch", "receiverStatus", "Quick Start", "quickStartList", "Apply Setup", "Custom Presets", "customPresetLabel", "customPresetList", "saveCustomPreset", "Import Custom", "Operator Overview", "operatorOverview", "workflowStrip", "capabilityGrid", "data-open-tab", "overviewFrequency", "overviewVfos", "overviewAlerts", "overviewDecoders", "Preset Navigator", "presetSearch", "presetModeFilter", "presetCategoryFilter", "presetSummary", "Band Map", "bandMap", "Run Queue", "runQueueList", "addRunQueueCurrent", "exportRunQueue", "clearRunQueue", "Scan Plans", "scanPlanSelect", "runScanPlan", "scanPlanOutput", "VFO Deck", "Add VFO", "Export VFOs", "Import VFOs", "vfoList", "vfoActivity", "Activity Intelligence", "activityList", "activitySummary", "activityMinPeak", "Promote", "Watch Rules", "alertRuleLabel", "alertRules", "alertEvents", "Save Watch", "Scan Range", "Bookmarks", "bookmarkSearch", "bookmarkCategory", "bookmarkImportFile", "Import Bookmarks", "Decoders", "POCSAG", "RDS", "Transcription", "decoderStatus", "decoderEvents", "decoderPlanOutput", "Connect HackRF", "RX Readiness", "readinessOutput", "Test RX/Sweep", "USB / Serial", "Diagnostics", "diagnosticsOutput", "Waterfall", "Sweep", "Capture", "Settings", "js/api.js", "css/style.css"]:
        require(token in html, f"missing SDR UI token {token!r}")
    for token in [".dashboard-band", ".operator-overview", ".workflow-strip", ".status-pill", ".capability-grid", ".capability-card", ".metric-strip", ".preset-card", ".preset-navigator", ".band-map", ".band-row", ".band-fill", ".run-queue"]:
        require(token in style, f"missing SDR visual style token {token!r}")
    for token in ["renderOperatorOverview", "overviewFrequency", "overviewDecoders", "dashboardMetrics", "workflowStrip", "renderPresetNavigator", "presetModeFilter", "presetCategoryFilter", "renderBandMap", "applyBandMapSelection", "data-band-start", "renderRunQueue", "addRunQueueCurrent", "applyRunQueueItem", "exportRunQueue", "runQueueKey", "activateTab", "data-open-tab"]:
...
def validate_integration() -> None:
    ...
    require("sdrServiceToggle" in web_html and "sdrServiceKill" in web_html and "sdrServiceStatus" in web_html, "main WebUI must expose SDR service memory-saving controls")
    require("/api/sdr/status" in web_server and "/api/sdr/control" in web_server and "SDR_SERVICE_NAME" in web_server, "WebUI server must expose SDR service status/control APIs")
    require('"disable", "--now", SDR_SERVICE_NAME' in web_server and "enable-start" in web_server, "SDR service control must support memory-saving disable and re-enable start")
    require("loadSdrServiceStatus" in web_js and "controlSdrService" in web_js and "applySdrServiceData" in web_js, "main WebUI must wire SDR service controls")
    require("controlSdrService(sdrServiceToggle.checked ? 'enable-start' : 'disable')" in web_js, "SDR toggle must enable/start or disable/stop the service")
    require("SDR Service" in readme and "disable --now ktox-sdr.service" in readme, "README must document the SDR memory-saving toggle")
